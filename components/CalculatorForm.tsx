...
const InputField: React.FC<InputFieldProps> = ({ id, label, icon, value, onChange, step = "0.01", unit = '' }) => (
    <div>
        ...
        <div className="relative">
            ...
            <input type="number" id={id} name={id} value={value} onChange={onChange} step={step} className="w-full pl-10 pr-12 py-2 bg-white/10 text-white border border-white/20 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition duration-200 placeholder-gray-400" placeholder="0"/>
            {unit && <span className="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-300 text-sm">{unit}</span>}
        </div>
    </div>
);
...
const CheckboxField: React.FC<CheckboxFieldProps> = ({ id, label, checked, onChange }) => (
    <div className="flex items-center">
        ...
        <label htmlFor={id} className="ml-3 block text-sm text-gray-200">{label}</label>
    </div>
);
...
const MinimalistInputField: React.FC<{id: string, value: number, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void, unit: string}> = ({id, value, onChange, unit}) => (
    <div className="relative">
        <input 
            ...
            onChange={onChange}
            step="0.01"
            className="w-full py-2 bg-transparent text-white border-b-2 border-white/20 focus:outline-none focus:border-blue-400 transition duration-200 placeholder-gray-400"
            placeholder="0"
        />
        <span className="absolute inset-y-0 right-0 flex items-center pr-1 text-gray-300 text-sm">{unit}</span>
    </div>
)
...
const DealForm: React.FC<DealFormProps> = ({ inputs, onInputChange, onInvestorChange, onInvestorCountChange, onReset, onAnalyze, totalParticipation }) => {
        ...
        <div className="bg-black/20 backdrop-blur-xl p-4 sm:p-6 rounded-2xl shadow-lg border border-white/20 max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-4">
                ...
                <Section title="Suministros" icon="fa-solid fa-lightbulb">
                    <p className="text-xs text-gray-400 -mt-2 mb-2">Coste estimado de alta: 250€ + 21% IVA por suministro.</p>
                ...
                <Section title="Inversores" icon="fa-solid fa-users">
                    <div>
                        <label htmlFor="investorCount" className="block text-sm font-medium text-gray-300 mb-1">Número de Inversores</label>
                    ...
                    {inputs.investors.map((investor, index) => (
                        <div key={investor.id} className="p-3 sm:p-4 bg-black/10 rounded-lg mt-2 space-y-4 border border-white/10">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                                <div>
                                    <label className="block text-xs text-gray-400 mb-1">Participación</label>
                             ...
                             <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-2">
                                <div>
                                    <label className="block text-xs text-gray-400 mb-1">% Financiación</label>
                                ...
                                <div>
                                    <label className="block text-xs text-gray-400 mb-1">TAE</label>
                                ...
                                <div>
                                    <label className="block text-xs text-gray-400 mb-1">Costes Asoc.</label>
